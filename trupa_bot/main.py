...
def start(update, context):
    msg = (
        "\U0001F4E1 Adaptive AI SignalBot Activated!\n\n"
        "This bot uses XGBoost with dynamic retraining every 3 minutes based on the latest 5000 candles.\n\n"
        "• Dynamic TP/SL: TP = $3, SL = $2\n"
        "• Adaptation Cycle: Every 3 minutes\n"
        "• Signal Type: Model-based only\n\n"
        "Use /predict to check for a signal.\n"
        "Use /monitor to see the model health."
    )
...
def monitor(update, context):
    if not latest_stats:
        ...
        return
    msg = (
        f"\U0001F4CA Model Health Check\n"
        f"Trades: {latest_stats['trades']}\n"
        f"Accuracy: {latest_stats['accuracy']:.2f}\n"
        f"Precision: {latest_stats['precision']:.2f}\n"
        f"Recall: {latest_stats['recall']:.2f}\n"
        f"F1-Score: {latest_stats['f1']:.2f}"
    )
    context.bot.send_message(chat_id=update.effective_chat.id, text=msg, parse_mode="Markdown")
